<template>
  <div class="m-10 p-5">
    <h4 class="text-3xl font-bold">Form edit</h4>
    <br />
    <div class="flex">
    ...
    <div>
      <div class="flex w-full">
        ...
        &nbsp;
        <div class="flex flex-col w-[13vw]">
...
const detail = ref([]);
const detailSaved = ref([]);

const formQty = ref(0);
const formService = ref("");
const formIdTemp = ref(null);
...
const getDetailInvoice = async () => {
  const { data } = await geInvoice(route.params?.id);

  subject.value = data?.subject;
  percent_tax.value = data?.percent_tax;
  from_destination_id.value = data?.from_destination_id;
  to_destination_id.value = data?.to_destination_id;

  detail.value = data?.detail?.map((item) => {
    return {
      id: item.id,
      service: item.service,
      quantity: item.quantity,
      subAmount: item.total_subamount,
    };
  });
};
...
const setSelectedItem = (item) => {
  ...
  formService.value = item.service.id;
  formIdTemp.value = item.id;
};

const resetForm = () => {
  formQty.value = 0;
  formService.value = null;
  formIdTemp.value = null;
};

const addToDetail = () => {
  if (!formIdTemp.value) {
    console.log("first kosong", formIdTemp.value);
    const service = selectService.value.find(
      (serv) => serv.id === formService.value
    );
    detail.value.push({
      service,
      quantity: formQty.value,
      subAmount: nf(service, "unit_price", 0) * formQty.value,
    });
  } else {
    console.log("first edit", formIdTemp.value);
    detail.value = detail.value.map((item) => {
      if (formIdTemp.value === item.id) {
        let service = formService.value;
        if (!service?.id) {
          service = selectService.value.find(
            (serv) => serv.id === formService.value
          );
        }
        return {
...
const onSave = async () => {
  ...
  const { status } = await updateInvoice(route.params.id, payload);
